...
from constants import __program__, __version__, TYPE_LIST, RE_PREFIX, RE_TYPE, RE_LIST, RE_SPACE
...
def check_for_columns(text, threshold=0.1):
    ...
    filter_prfx = {k: v for k, v in prfx_ranges.items() if len(v)/total_prfx > threshold}
    ...
    filter_type = {k: v for k, v in type_ranges.items() if len(v)/total_type > threshold and k > min_prfx}
    ...
    filter_spce = {k: v for k, v in spce_ranges.items() if len(v)/total_spce > threshold and k > max(min_prfx, min_type)}
        ...
        if not line:
            ...
            if matchl:
                ...
                if start > 0 and not upstr.isspace():
                    logging.debug("Ignore 'prefix' '{}' b/c upstream is not space: '{}'".format(match, upstr))
                else:
                    for match in matchl:
                        ...
                        if in_bounds(start, min_prfx, min_spce):
                            ...
                            last = max(last, end)
                            logging.debug("match 'prefix' at position {}: '{}'".format(start, match))
            ...
            for match in re.finditer(RE_TYPE, line, flags=re.IGNORECASE):
                ...
                if in_bounds(start, max(min_prfx, min_type, last), min_spce):
                    ...
                    last = max(last, end)
                    logging.debug("match 'type' at position {}: '{}'".format(start, match))
            ...
            if matchl:
                for match in matchl:
                    ...
                    if in_bounds(start, min_spce):
                        ...
                        logging.debug("match 'docstring' at position {}: '{}'".format(start, match))
                    # relax the position requirement if not sharing the line with a prefix
                    elif not prfx_matches:
                        spce_matches.append(match)
                        logging.debug("loose match 'docstring' at position {}: '{}'".format(start, match))
            # if the docstring is not preceeded by 3+ whitespaces, assume it starts at 'min_spce' if not overlapping a prefix
            else:
                if len(line) > min_spce and line[min_spce-1].isspace():
                    match = line[min_spce:]
                    spce_matches.append(match)
                    logging.debug("assume 'docstring' at position {}: '{}'".format(min_spce, match))
...
def convert_to_cwlgen(arg_list):
    ...
    for arg in arg_list:
        ...
        cwl_binding = CommandLineBinding(
            prefix=prfxm, 
            position=0, 
        )
...
if __name__ == '__main__':
    ...
    with open(sys.argv[1], 'r') as fh:
        text = [line.rstrip() for line in fh.readlines()]

    columns = check_for_columns(text)
    arg_list = build_args(text, columns)
    arg_list = post_process(arg_list)

    cwl_args = convert_to_cwlgen(arg_list)
